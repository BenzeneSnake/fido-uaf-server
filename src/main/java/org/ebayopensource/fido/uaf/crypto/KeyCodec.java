...
public class KeyCodec {
	...
	/**
	 * 把 EC 公鑰（橢圓曲線上的點）轉成「Raw bytes」形式（未壓縮格式）
	 *
	 * @param pub
	 * @return
	 * @throws IOException
	...
	public static byte[] getKeyAsRawBytes(
		...
		bos.write(0x04);
		bos.write(asUnsignedByteArray(pub.getQ().getAffineXCoord().toBigInteger()));
		bos.write(asUnsignedByteArray(pub.getQ().getAffineYCoord().toBigInteger()));
		raw = bos.toByteArray();
		logger.info("Raw key length:" + raw.length);
		return raw;
	}
